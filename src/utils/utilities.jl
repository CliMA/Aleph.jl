...
function compute_kinetic!(
    ...
    @assert eltype(ᶠu₃) <: C3
    ᶜg³³_component = g³³_field(ᶜκ)
    ᶠu₃_component = ᶠu₃.components.data.:1
    @. ᶜκ =
        (
            dot(CT12(ᶜuₕ), C12(ᶜuₕ)) +
            2 * dot(CT3(ᶜuₕ), ᶜinterp(ᶠu₃)) +
            ᶜg³³_component * ᶜinterp(ᶠu₃_component^2)
        ) / 2
end
