...
function plot_contours!(place, var; n_contours = 22, kwargs...)
    ...
    data_mid = data_min < data_avg_int < data_max ? data_avg_int : data_avg
...
make_contour_plots(get_vars, short_names, output_paths, output_name) =
    make_plots_generic(
        output_paths,
...
const PeriodicTopographyTest2D = Union{
    Val{:plane_no_topography_long_float64_test},
    Val{:plane_cosine_hills_float64_test},
    Val{:gpu_plane_cosine_hills_high_res_float64_test},
}
const PeriodicTopographyTest3D = Union{
    Val{:gpu_extruded_plane_cosine_hills_float32_test},
    Val{:gpu_box_cosine_hills_float32_test},
}
const TopographyTest =
    Union{MountainTest2D, PeriodicTopographyTest2D, PeriodicTopographyTest3D}

function make_plots(val::TopographyTest, output_paths::Vector{<:AbstractString})
    simdirs = SimDir.(output_paths)
    is_mountain_test = val isa MountainTest2D
    is_3d = val isa PeriodicTopographyTest3D
    z_top = 13e3 # The Rayleigh sponge starts at 13 km and shouldn't be plotted.
        ...
        Iterators.flatmap((horizontal_average, vertical_average)) do average
            Iterators.flatmap(("uaerror", "waerror")) do short_name
                Iterators.map(simdirs) do simdir
                    ...
                    var = window(var, z_dim_name(var); right = z_top)
    ...
    for velocity_short_name in ("ua", "wa")
        is_mountain_test || break
        short_names = velocity_short_name .* ("error", "", "predicted")
        output_name = "final_mountain_closeup_" * velocity_short_name
        make_contour_plots(short_names, output_paths, output_name) do short_name
            Iterators.flatmap(simdirs) do simdir
                var = get(simdir; short_name)
                var = window(var, "x"; left = 35e3, right = 65e3)
                var = is_3d ? slice(var; y = 0) : var
                var = slice(var; time = Inf)
                z_max_values_to_plot =
                    endswith(short_name, "error") ? (1e3, z_top) : (z_top,)
                Iterators.map(z_max_values_to_plot) do z_max
    ...
    for velocity_short_name in ("ua", "wa")
        GC.gc() # this is needed to avoid out-of-memory errors for 3D vars
        GC.gc()
        GC.gc()
        GC.gc()
        short_names = velocity_short_name .* ("error", "", "predicted")
        output_name = "slice_summary_" * velocity_short_name
        make_contour_plots(short_names, output_paths, output_name) do short_name
            Iterators.flatmap(simdirs) do simdir
                var = get(simdir; short_name)
                var = window(var, z_dim_name(var); right = z_top)
                var = is_3d ? slice(var; y = 0) : var
                hours_to_plot = if endswith(short_name, "predicted")
                    (Inf,) # predicted value is constant, so we only need 1 plot
                elseif last(var.dims["time"]) > 24 * 60 * 60
                    (1, 2, 24, Inf)
                else
                    (1, 2, 3, 24)
                end
                Iterators.map(hours_to_plot) do n_hours
                    slice(var; time = n_hours * 60 * 60)
                end
            end
        end
    end
